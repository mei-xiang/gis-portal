<template>
  <div class="geo">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>坐标是否在指定多边形内</el-breadcrumb-item>
    </el-breadcrumb>
    <h1 class="title">坐标是否在指定多边形内</h1>
    <h2>接口使用说明</h2>
    <h3>坐标是否在指定多边形内接口</h3>
    <h4>请求地址</h4>
    <p>http://192.168.44.152:8080/v1/point/is-point-in-polygon</p>
    <h4>请求方法</h4>
    <p>POST</p>
    <h4>请求</h4>
...
<script>
import { getPolygonData } from 'network/point'
export default {
  data() {
    return {
      isRun: false,
  ...
  methods: {
    run() {
      ...
      // 通过类整合数据
      getPolygonData(this.distanceObj).then(res => {
        this.content = res
      })
    }
  }
